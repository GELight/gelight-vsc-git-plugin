...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('gelight.editCommitMessage', (ctx?: { webviewSection?: string; commitHash?: string }) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('gelight.deleteCommit', (ctx?: { webviewSection?: string; commitHash?: string }) => {
      const hash = ctx?.commitHash ?? _selectedCommitHash;
      if (hash) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('gelight.resetToCommit', (ctx?: { webviewSection?: string; commitHash?: string }) => {
      const hash = ctx?.commitHash ?? _selectedCommitHash;
      if (hash) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('gelight.copyCommitHash', (ctx?: { webviewSection?: string; commitHash?: string }) => {
      const hash = ctx?.commitHash ?? _selectedCommitHash;
      if (hash) {
        vscode.env.clipboard.writeText(hash);
        vscode.window.showInformationMessage(`Copied: ${hash.substring(0, 7)}`);
      }
    }),
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('gelight.fixup', (ctx?: { commitHashes?: string[] }) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('gelight.squash', (ctx?: { commitHashes?: string[] }) => {
      const hashes = ctx?.commitHashes ?? _selectedCommitHashes;
      if (hashes.length >= 2) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('gelight.softReset', (ctx?: { commitHashes?: string[] }) => {
      const hashes = ctx?.commitHashes ?? _selectedCommitHashes;
      if (hashes.length > 0) {
  ...
  // --- File-level command ---
  context.subscriptions.push(
    vscode.commands.registerCommand('gelight.softResetFileFromCommit', (ctx?: { commitHash?: string; filePath?: string }) => {
      const hash = ctx?.commitHash ?? _selectedCommitHash;
  ...
  context.subscriptions.push(gitService, gitWatcher, gitGraphProvider, commitChangesProvider);
}
