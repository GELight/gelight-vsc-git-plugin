...
export class GitCliService {
   ...
   */
  private async _exec(args: string[], env?: Record<string, string>): Promise<string> {
   ...
   */
  public async editCommitMessage(commitHash: string, newMessage: string): Promise<void> {
    // For HEAD commit, use amend directly
    const headHash = await this._exec(['rev-parse', 'HEAD']);
    if (headHash.startsWith(commitHash) || commitHash.startsWith(headHash.substring(0, 7))) {
  ...
  public async fixup(commitHashes: string[]): Promise<void> {
    if (commitHashes.length < 2) { return; }
   ...
   */
  public async softResetFileFromCommit(commitHash: string, filePath: string): Promise<void> {
    const headHash = await this._exec(['rev-parse', 'HEAD']);
    const isHead = headHash.startsWith(commitHash) || commitHash.startsWith(headHash.substring(0, 7));
  ...
  public async getTags(): Promise<Map<string, string[]>> {
    const output = await this._exec(['tag', '-l', '--format=%(objectname:short) %(refname:short)']);
  ...
  private _createTempScript(content: string): string {
    ...
    const scriptPath = path.join(tmpDir, `gelight-git-${Date.now()}-${Math.random().toString(36).slice(2)}.sh`);
